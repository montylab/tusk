---
name: Exhaustive Test Plan Generation
description: Systematic process for generating 1:1 test plan coverage for every source file in a codebase, with strict anti-skip guardrails.
---
...
1. Run a recursive file listing command to produce an **exhaustive manifest**:
   ```powershell
   # Windows PowerShell
   Get-ChildItem -Path src -File -Recurse -Include *.vue,*.ts,*.js,*.tsx,*.jsx | Resolve-Path -Relative
   ```
   ```bash
   # Unix / macOS
   find src -type f \( -name '*.vue' -o -name '*.ts' -o -name '*.js' \)
   ```
2. Save the output as a checklist in `task.md`. **Every file becomes a checkbox item.**
3. Count the files. This number is the **target** — you are done only when the test-plan directory contains at least this many mappings.
...
1. **One file → one plan.** Every source file gets its own `.md` document. Never combine two components into one document just because they are similar (e.g., `TrashBasket.vue` and `TrashBasketRound.vue` are TWO separate plans).
