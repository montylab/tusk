<script setup lang="ts">
  ...
  const { currentZone, activeDraggedTaskId, isDragging, registerZone, unregisterZone, updateZoneBounds, startDrag } =
    useDragOperator()
  ...
  const calculateDropData = (x: number, y: number, task: Task) => {
    ...
    const tasks = Array.from(
      contentRef.value.querySelectorAll('.task-group:not(.bottom-indicator-group)')
    ) as HTMLElement[]
    ...
    <div ref="contentRef" class="pile-content">
      <TransitionGroup name="task-list">
        ...
        <div v-for="(task, index) in tasks" :key="task.id" class="task-group">
          ...
          <div
            ...
            :style="[
              getChaoticStyle(task),
              {
                '--category-color':
                  task.color ||
                  categoriesStore.categoriesArray.find((c) => c.name === task.category)?.color ||
                  'var(--color-default)'
              }
            ]"
          ...
          >
            <TaskItem :task="task" @edit="emit('edit', $event)" />
          </div>
        </div>
