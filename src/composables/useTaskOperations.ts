...
export function useTaskOperations(
  ...
  const onMouseMove = (e: MouseEvent) => {
    ...
    if (mode.value === 'drag') {
      ...
      snappedStart = Math.min(
        Math.max(config.startHour, snappedStart),
        initialStart.value + initialDuration.value / 60 - 0.25
      )
