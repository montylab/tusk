		...
		return taskList.value
			.map((t) => {
				...
				return {
					...
					layoutEnd:
						(t.id === activeTaskId.value ? startTime : displayStart) + (t.id === activeTaskId.value ? duration : displayDuration) / 60
				}
			})
	...
	const layoutTasks = computed(() => {
		...
		for (const cluster of clusters) {
			...
			cluster.forEach((task, index) => {
				...
				const hHeight = unref(config.hourHeight)

				result.push({
					...task,
					isOverlapping: isClusterOverlapping && !isSelfDragging,
					style: {
						top: `${(task.displayStart - config.startHour) * hHeight}px`,
						height: `${(task.displayDuration / 60) * hHeight - 1}px`, // we remove 1px to avoid double lines
						left: `${colIndex * width}%`,
						width: `${width}%`,
