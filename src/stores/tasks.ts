...
export const useTasksStore = defineStore('tasks', () => {
    ...
    return {
        ...
        // --- DND Management ---
        async manageTaskRelocation(source: string, dest: string, task: Task, dropData: any) {
            // Destination: Trash
            if (dest === 'trash') {
                if (source.startsWith('calendar-day-')) {
                    const date = source.replace('calendar-day-', '')
                    await deleteScheduledTask(task.id, date)
                } else if (source === 'todo') {
                    await deleteTodo(task.id)
                } else if (source === 'shortcut') {
                    await deleteShortcut(task.id)
                }
                return
            }
            ...
            if (dest.startsWith('calendar-day-')) {
                ...
                if (source.startsWith('calendar-day-')) {
                    ...
                    if (task.id.toString().startsWith('temp-')) {
                        ...
                        await createScheduledTask({ ...taskData, date, startTime: time, duration })
                    } else {
            ...
            // Destination: Todo
                ...
                if (source.startsWith('calendar-day-')) {
                    ...
                    const date = source.replace('calendar-day-', '')
                    await moveCalendarToTodo(task.id, date, order)
                } else if (source === 'todo') {
                    // Todo → Todo: Reorder
                    if (index !== undefined) {
                        await reorderTodo(task.id, index)
                    }
                } else if (source === 'shortcut') {
                    // Shortcut → Todo: Copy
                    await copyShortcutToTodo(task.id, order)
                }
            }
            ...
            // Destination: Shortcut
                ...
                if (source.startsWith('calendar-day-')) {
                    ...
                    const date = source.replace('calendar-day-', '')
                    await moveCalendarToShortcut(task.id, date, order)
                } else if (source === 'todo') {
                    // Todo → Shortcut: Move (promote)
                    await moveTodoToShortcut(task.id, order)
                } else if (source === 'shortcut') {
                    // Shortcut → Shortcut: Reorder
                    if (index !== undefined) {
                        await reorderShortcut(task.id, index)
                    }
                }
            }
        },
